...
import mintlin.serializer.StrReader
...
object Json {
    ...
    private fun parseArray(s: StrReader, context: Context = Context.DEFAULT): Any {
        ...
        var outNumber: ArrayList<Double>? = null
        s.skipExpect('[')
        array@ while (true) {
            when (s.skipSpaces().read()) {
            ...
            if (out == null && context.optimizedNumericLists && (v in '0'..'9' || v == '-')) {
                ...
                if (outNumber != null) {
                    outNumber.forEach { out.add(it) }
                    outNumber = null
                }
                out.add(parse(s, context))
            }
        }
        return outNumber ?: out ?: FastArrayList<Any>()
    }
	...
	fun stringify(obj: Any?, b: StringBuilder) {
		when (obj) {
			...
			else -> throw AssertionError("Don't know how to serialize $obj") //encode(ClassFactory(obj::class).toMap(obj), b)
		}
	}
	...
	fun stringifyPretty(obj: Any?, b: Indenter) {
		when (obj) {
			...
			is CustomSerializer -> b.inline(StringBuilder().apply { obj.encodeToJson(this) }.toString())
				...
				throw AssertionError("Don't know how to serialize $obj")
