...
import mintlin.format.json.NumberParser
...
class StrReader(val str: String, val file: String = "file", override var pos: Int = 0) : BaseStrReader() {
    ...
    fun readStringLit(reportErrors: Boolean = true): String {
        ...
        when (quotec) {
            '"', '\'' -> Unit
            else -> throw AssertionError("Invalid string literal")
        }
        var closed = false
        loop@ while (hasMore) {
            when (val c = read()) {
    ...
    fun readFloats(list: FastArrayList<Float> = FastArrayList<Float>(7)): FastArrayList<Float> {
        while (!eof) {
            val pos0 = pos
    ...
    fun readInts(list: FastArrayList<Int> = FastArrayList<Int>(7)): FastArrayList<Int> {
        while (!eof) {
            val pos0 = pos
